...
#    month_statement_period = {}
    date_format = str(self.pool.get('account.coda.import').browse(cr, uid, ids[0]).date_format)
...
#        st_line_1 = {}
    ...
    for key in month_statement.keys():
        ...
        for line in month_statement[key]:
            ...
            st_line = {}
            line_name = i
            name = line_splited[12]
            date = line_splited[11]
            st_line['date'] = line_splited[11]
            st_line['name'] = line_splited[12]
            st_line['extra_note'] = line_splited[12]
            st_line['ref'] = line_splited[14]
            amount = line_splited[17].replace(' ','').replace(',','.')
            ...
            check_ids = self.pool.get('account.bank.statement.line').search(cr,uid,[('name','=',name),('date','=',date),('amount','=',amount)])
            if check_ids:
                continue
