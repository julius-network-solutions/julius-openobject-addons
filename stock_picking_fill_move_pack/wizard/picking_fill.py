...
class stock_picking_fill(osv.osv_memory):
    ...
    def _get_vals_pack(self, cr, uid, current, picking_id, location_id, location_dest_id, address_id=False, context=None):
        res = []
        move_obj = self.pool.get('stock.move')
        if not current.pack_ids:
            raise osv.except_osv(_('Invalid action !'), _('There are no pack to add, please select at least 1 pack to add to the picking !'))
        for pack in current.pack_ids:
            ...
            for product_id in prod_prod.keys():
                ...
                if quantity and quantity > 0:
                    result_vals = move_obj.onchange_product_id(cr, uid, [], prod_id=product_id, loc_id=location_id, loc_dest_id=location_dest_id, address_id=address_id or False)
                    line_vals = result_vals and result_vals.get('value') or False
                    if line_vals:
                        line_vals.update({'picking_id': picking_id})
                        line_vals.update({'product_id': product_id})
                        line_vals.update({'product_qty': quantity})
                        line_vals.update({'product_uos_qty': False})
                        line_vals.update({'product_uos': False})
                        line_vals.update({'tracking_id': tracking_id})
                        res.append(line_vals)
        return res
    ...
    def _get_vals(self, cr, uid, current, context=None):
        ...
        if current.type_id.code == 'pack':
            res = self._get_vals_pack(cr, uid, current, picking_id=picking_id, location_id=location_id, location_dest_id=location_dest_id, address_id=address_id, context=context)
        return res
    
    _columns = {
        'location_id': fields.many2one('stock.location', 'Location'),
        'pack_ids': fields.many2many('stock.tracking', 'pack_fill_picking_rel', 'wizard_id', 'pack_id', 'Packs', domain=[('parent_id', '=', False),('state', '=', 'close')]),
    }
