...
class multiple_edition_settings(orm.TransientModel):
    ...
    _columns = {
        'models_ids': fields.many2many('ir.model',
            'multiple_edition_settings_mode_rel',
            'multiple_edition_id', 'model_id', 'Models'),
    }
    ...
    def update_field(self, cr, uid, vals, context=None):
        ...
        unlink_ids = action_obj.search(cr,  uid, [('res_model' , '=', 'multiple.edition')])
        ...
        for model in read_datas:
            act_id = action_obj.create(cr, uid, {
                 'name': "%s Multiple Edition" % model['name'],
