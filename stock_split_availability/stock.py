...
class stock_move(orm.Model):
    ...
    def _get_context_check(self, cr, uid, move, what=('in'), context=None):
        return {
            'shop': False,
            'warehouse': False,
    ...
    def _get_specific_available_qty(self, cr, uid, move, context=None):
        if context is None:
            context = {}
        c = context.copy()
        c.update(self._get_context_check(cr, uid, move, ('in'), context=context))
        ...
        c.update(self._get_context_check(cr, uid, move, ('out'), context=context))
        stock = self.pool.get('product.product').get_product_available(cr, uid, [move.product_id.id], context=c)
