...
class AccountInvoiceLine(models.Model):
    ...
    def create(self, vals):
        ...
        # Create new invoice line for launch costs if necessary
        ...
        if not invoice_line.is_cost_line:
            for sale_line in invoice_line.sale_line_ids:
                if sale_line._launch_line_to_create():
                    price_unit = sale_line.launch_costs or 0
                    
                    # Create order line linked to order with launch costs product
                    sale_order_line_env = self.env['sale.order.line']
                    vals = {'product_id': product.id,
                            'name': product.name,
                            'order_id': sale_line.order_id.id,
                            'product_uom': product.uom_id.id}
                    launch_costs_sale_order_line = sale_order_line_env.new(vals)
                    launch_costs_sale_order_line.product_id_change()

                    # Prepare invoice from sale order line created
                    vals = launch_costs_sale_order_line._prepare_invoice_line(1)
                    ...
                    if vals:
                        ...
                        if new_invoice_line:
                            ...
                            sale_line.write({'launch_costs_line_id': new_invoice_line.id})
