...
class stock_production_lot(orm.Model):
    ...
    def _get_current_location(self, cr, uid, ids,
        ...
        for prodlot in self.browse(cr, uid, ids, context=context):
            ...
            if move_ids:
                ...
                result[prodlot.id] = move.location_dest_id.id
        return result
    ...
    _columns = {
        'current_location_id': fields.function(_get_current_location,
            type='many2one', relation='stock.location',
            string='Current Location', store={
